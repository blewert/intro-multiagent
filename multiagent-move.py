...
## Horde Labs
## ---
## A simple example which involves drawing multiple agents moving around an environment.
## 
...
agents = [ Agent(canvas, (0.0, 0.0), 10) for i in range(0, 15) ];
...
while True:
	#And then, each step move each agent forward and relocate them.
	for agent in agents:
		agent.fd(1);
		agent.update();
